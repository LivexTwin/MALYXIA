...
{#if variant === "compact"}
  <div class="relative aspect-square w-full overflow-visble">
    ...
    {#key index}
      <ShopifyImage
       ...
       height: 800
        ...
        classList=" w-[320px] md:w-full h-full mx-auto object-contain"
      />
    {/key}
    ...
    {#if images.length > 1}
      <button
        on:click={prev}
        class="absolute left-2 top-1/2 -translate-y-1/2 -translate-x-1/2 w-12 h-12 flex items-center justify-center"
      >
        ‹
      </button>
      ...
      <button
        on:click={next}
        class="absolute right-2 top-1/2 -translate-y-1/2 translate-x-1/2 w-12 h-12 flex items-center justify-center"
      >
        ›
      </button>
    {/if}
